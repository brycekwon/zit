...
if [[ -z "${ZIT_MODULES_HOME}" ]]; then
    export ZIT_MODULES_HOME="${ZDOTDIR:-${HOME}}"
fi
...
function zit-install() {
    ...
    local module_dir="${ZIT_MODULES_HOME}/${1}"
...
function zit-load() {
    ...
    source "${ZIT_MODULES_HOME}/${1}/${2}" || return 1
}

# update plugin from remote url
function zit-upgrade() {
    # pull new changes into each module repository
    for i in $(command ls $ZIT_MODULES_HOME); do
        pushd "$ZIT_MODULES_HOME/${i}" > /dev/null || continue
